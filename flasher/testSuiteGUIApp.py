...
AUTO_START_WAIT_BEFORE_DISCONNECT = 40 #wait n seconds before considering a disconnect to handle switch to FASTBOOT
	...
	def _checkAndTriggerDeviceChanges(self, dt):
		...
		for uid, deviceDescriptor in self.deviceDescriptors.iteritems():
			...
			if lastKnownState != currentState: #if the state is different since last time
				print "state Change: lastKnown: " + str(lastKnownState) + " current " + str(currentState) + " elapsed " + str(elapsedTime)
